#Universal 1994c. Standardized IF the attributive noun (genitive, esp. when morphologically unmarked)  precedes the head noun, THEN (i) nominal affixes (for case, number, gender/class) and adpositions are postposed (suffixes, postpositions), (ii) pronominal possessives precede the head noun, (iii) attributive adjectives precede the head noun, (iv) verbal affixes (person, number)  and pronouns (for subject) precede the verb, (v) direct objects (accusative, esp. when morphologically unmarked) precede the verb.

#non-affixed Gen-N > verbal prefixes pers/num

#GB065:1 & GB430:0 & GB432:0 > GB090:1 | GB092:1
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB065", "GB430", "GB432", "GB090", "GB092"))
...
#making sure the second set of conditions is met: GB090:1 | GB092:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB090[i] == 1 | data_frame_compl_pruned$GB092[i] == 1 )
