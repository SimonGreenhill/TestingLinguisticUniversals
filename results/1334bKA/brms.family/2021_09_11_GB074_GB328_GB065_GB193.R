#Universal 1334b. "Prepositional Noun-Modifier Hierarchy (PrNMH): If a language is prepositional, then
...
#Adp-N & Rel-N & Gen-N > Adj-N

#GB074:1 & GB328:1 & GB065:1 > GB193:1
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB074", "GB328", "GB065", "GB193"))

data_frame_compl <- data_frame[complete.cases(data_frame),]

# prepare datafile

#making sure the first set of conditions is met: GB074:1 & GB328:1 & GB065:1
for(i in 1:nrow(data_frame_compl)){
  if(data_frame_compl$GB074[i] == 1 & data_frame_compl$GB328[i] == 1 & data_frame_compl$GB065[i] == 1)
