#Universal 179a. There is a hierarchy: Number > Gender > Case, such that if some target agrees in one category on the hierarchy then it will also agree in all categories higher on the hierarchy.

#gender Adj > number Adj
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB170", "GB184"))
...
#making sure the first set of conditions is met: GB170:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB170[i] == 1)
  {data_frame_compl_pruned$Condition_1[i] <- 1}
  else(data_frame_compl_pruned$Condition_1[i] <- 0)
}


#making sure the second set of conditions is met: GB184:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB184[i] == 1)
