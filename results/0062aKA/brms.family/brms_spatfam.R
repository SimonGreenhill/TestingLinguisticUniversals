...
glottolog_w_fams <- read.csv("Glottolog_v4.8.tsv", sep = "\t")
...
datfra <- datfra[!datfra$family == "",] # no isolates allowed

fams <- as.vector(sort(table(datfra$family)))
names(fams) <- names(sort(table(datfra$family)))
too_small <- fams[fams < 5]
datfra <- datfra[!datfra$family %in% names(too_small),] # no small families allowed
