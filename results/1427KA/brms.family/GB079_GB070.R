...
for(i in 1:nrow(GB079_GB070_compl)){
  if((GB079_GB070_compl$GB079[i] == '0') & (GB079_GB070_compl$GB090[i] == '0') & (GB079_GB070_compl$GB092[i] == '0') & (GB079_GB070_compl$GB094[i] == '0') & (GB079_GB070_compl$GB430[i] == '0') & (GB079_GB070_compl$GB431[i] == '0')) {GB079_GB070_compl$No_prefixes[i] <- 1}
  else(GB079_GB070_compl$No_prefixes[i] <- 0)
}

for(i in 1:nrow(GB079_GB070_compl)){
  if((GB079_GB070_compl$GB072[i] == '1') | (GB079_GB070_compl$GB073[i] == '1')) {GB079_GB070_compl$Cases[i] <- 1}
  else(GB079_GB070_compl$Cases[i] <- 0)
}

GB079_GB070_compl2 <- subset(x = GB079_GB070_compl, select = c("Language_ID", "No_prefixes", "Cases"))

# write files
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

write.table(GB079_GB070_compl2, file = "BT_data.txt", sep = "\t", quote = F, row.names = F, col.names = F)
