...
for(i in 1:nrow(GB409_GB133_compl_pruned)){
  if((GB409_GB133_compl_pruned$GB131[i] == '0') & (GB409_GB133_compl_pruned$GB132[i] == '1') & (GB409_GB133_compl_pruned$GB133[i] == '0')) {GB409_GB133_compl_pruned$SVO[i] <- 1}
  else(GB409_GB133_compl_pruned$SVO[i] <- 0)
}

for(i in 1:nrow(GB409_GB133_compl_pruned)){
  if(GB409_GB133_compl_pruned$GB409[i] == '0') {GB409_GB133_compl_pruned$not_erg[i] <- 1}
  else(GB409_GB133_compl_pruned$not_erg[i] <- 0)
}

GB409_GB133_compl_pruned2 <- subset(x = GB409_GB133_compl_pruned, select = c("Language_ID", "SVO", "not_erg"))
