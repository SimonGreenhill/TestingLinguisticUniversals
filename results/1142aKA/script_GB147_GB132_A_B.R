...
for(i in 1:nrow(GB147_GB132_compl_pruned)){
  if((GB147_GB132_compl_pruned$GB131[i] == '0') & (GB147_GB132_compl_pruned$GB132[i] == '1') & (GB147_GB132_compl_pruned$GB133[i] == '0')) {GB147_GB132_compl_pruned$Verb_medial[i] <- 1}
  else(GB147_GB132_compl_pruned$Verb_medial[i] <- 0)
}

for(i in 1:nrow(GB147_GB132_compl_pruned)){
  if(GB147_GB132_compl_pruned$GB147[i] == '1' | GB147_GB132_compl_pruned$GB302[i] == '1' | GB147_GB132_compl_pruned$GB304[i] == '1') {GB147_GB132_compl_pruned$Passive[i] <- 1}
  else(GB147_GB132_compl_pruned$Passive[i] <- 0)
}


GB147_GB132_compl_pruned2 <- subset(x = GB147_GB132_compl_pruned, select = c("Language_ID", "Passive", "Verb_medial"))
