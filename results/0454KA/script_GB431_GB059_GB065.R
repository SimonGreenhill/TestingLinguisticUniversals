#Universal 454. Personal possessive prefixes always imply a GN order, but not the converse; for  personal possessive suffixes there is no such rule. Alternative formulation Ultan 1978: 26: "An examination of the 12 languages in the sample having inalienable pronominal prefixes shows that all have a GN constituent order in possessive constructions. In other words, a prefix implies a constituent order of GN in cases of inalienable possession by pronominal affix." 

#inalienable possessive prefixes > Gen-N

#GB431:1 & GB059:1 > GB065:1
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB431", "GB059", "GB065"))
...
#making sure the first set of conditions is met: GB431:1 & GB059:1 
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB431[i] == 1 & data_frame_compl_pruned$GB059[i] == 1)
