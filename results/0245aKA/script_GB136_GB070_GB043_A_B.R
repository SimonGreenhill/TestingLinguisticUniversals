...
#Version A
#Nominal words are inflected AND constituent order is free

#Version B
#No inflection for nominal words and constituent order is rigid

#GB136:0 > sum(GB070:1,GB072:1,GB042:1,GB043:1,GB044:1,GB046:1,GB165:1,GB166:1) > 0
...
GB136_GB070_GB043 <- subset(x = GB_wide_strict, select = c("Language_ID", "GB136", "GB070","GB072","GB042", 
                                                           "GB043", "GB044", "GB046","GB165", "GB166"))
...
GB136_GB070_GB043 <- subset(x = GB136_GB070_GB043, select = c("Language_ID", "GB136", "Inflected_Noun"))
...
for(i in 1:nrow(GB136_GB070_GB043_compl_pruned)){
  if(GB136_GB070_GB043_compl_pruned$GB136[i] == '0'){GB136_GB070_GB043_compl_pruned$Free[i] <- 1}
  else(GB136_GB070_GB043_compl_pruned$Free[i] <- 0)
}

GB136_GB070_GB043_compl_pruned2 <- subset(x = GB136_GB070_GB043_compl_pruned, select = c("Language_ID", "Free", "Inflected_Noun"))

table(GB136_GB070_GB043_compl_pruned2$Free, GB136_GB070_GB043_compl_pruned2$Inflected_Noun)

# write files

setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

write.table(GB136_GB070_GB043_compl_pruned2, file = "BT_data.txt", sep = "\t", quote = F, row.names = F, col.names = F)
