#Universal 917c. Consistent OV languages are consistently left-branching (placing phrasal constituents before nonphrasal ones) while consistent VO languages are consistently right-branching.

#OV > Rel-N

#GB131:0 & (GB132:1 | GB133:1) > GB328:1
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB131", "GB132", "GB133", "GB328"))
...
for(i in 1:nrow(data_frame_compl_pruned)){
  if((data_frame_compl_pruned$GB132[i] == 1 | data_frame_compl_pruned$GB133[i] == 1) & data_frame_compl_pruned$GB131[i] == 0)
  {data_frame_compl_pruned$Condition_1[i] <- 1}
  else(data_frame_compl_pruned$Condition_1[i] <- 0)
}

#making sure the second set of conditions is met: GB328:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB328[i] == 1)
