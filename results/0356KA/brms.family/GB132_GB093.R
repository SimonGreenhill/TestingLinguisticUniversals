...
for(i in 1:nrow(GB132_GB093_compl)){
  if(((GB132_GB093_compl$GB091[i] == '1') & (GB132_GB093_compl$GB092[i] == '0')) & ((GB132_GB093_compl$GB093[i] == '1') | (GB132_GB093_compl$GB094[i] == '1'))) {GB132_GB093_compl$Agreement[i] <- 1}
  else(GB132_GB093_compl$Agreement[i] <- 0)
}

for(i in 1:nrow(GB132_GB093_compl)){
  if((GB132_GB093_compl$GB132[i] == '1') | (GB132_GB093_compl$GB133[i] == '1')) {GB132_GB093_compl$SV[i] <- 1}
  else(GB132_GB093_compl$SV[i] <- 0)
}

GB132_GB093_compl2 <- subset(x = GB132_GB093_compl, select = c("Language_ID", "Agreement", "SV"))

nrow(GB132_GB093_compl2)

# write files

setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

write.table(GB132_GB093_compl2, file = "BT_data.txt", sep = "\t", quote = F, row.names = F, col.names = F)
