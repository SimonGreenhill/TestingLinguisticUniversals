#Universal 1545. In verb-initial languages, the demonstrative, numeral, and qualifying  adjective follow the common noun in that order or its mirror image (Adj + Num + Dem). 

#V-initial > N-Dem-Num-Adj

#(GB131:1 & GB132:0 & GB133:0) > GB024: 2 & GB025: 2 & GB193: 2
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB131", "GB132", "GB133", "GB193", "GB025", "GB024"))
...
#making sure the second set of conditions is met: GB024: 2 & GB025: 2 & GB193: 2
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB024[i] == 2 & data_frame_compl_pruned$GB025[i] == 2 & data_frame_compl_pruned$GB193[i] == 2)
  {data_frame_compl_pruned$Condition_2[i] <- 1}
  else(data_frame_compl_pruned$Condition_2[i] <- 0)
}

table(data_frame_compl_pruned$Condition_1, data_frame_compl_pruned$Condition_2)
