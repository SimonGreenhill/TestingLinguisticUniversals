...
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB131[i] == 0 & data_frame_compl_pruned$GB132[i] == 0 & data_frame_compl_pruned$GB133[i] == 1)
...
for(i in 1:nrow(data_frame_compl_pruned)){
  summ <- sum(c(as.numeric(data_frame_compl_pruned$GB089[i]), as.numeric(data_frame_compl_pruned$GB091[i]), as.numeric(data_frame_compl_pruned$GB093[i]), as.numeric(data_frame_compl_pruned$GB432[i]), as.numeric(data_frame_compl_pruned$GB433[i])), na.rm = T)
  if(summ > 2 &  data_frame_compl_pruned$GB080[i] == 1){data_frame_compl_pruned$Condition_2[i] <- 1}
