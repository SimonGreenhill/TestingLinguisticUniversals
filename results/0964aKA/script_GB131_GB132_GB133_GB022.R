#Universal 964a. "VO languages exhibit a statistically greater tendency to place articles before the noun than OV languages do. To the opposite, the tendency to place the noun before the articles is higher among OV languages than it is among VO languages."

#VO > Art-N

#(GB131:1 | GB132:1) & GB133:0 > GB022:1
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB131", "GB132", "GB133", "GB022"))
...
for(i in 1:nrow(data_frame_compl_pruned)){
  if((data_frame_compl_pruned$GB131[i] == 1 | data_frame_compl_pruned$GB132[i] == 1) & data_frame_compl_pruned$GB133[i] == 0)
  {data_frame_compl_pruned$Condition_1[i] <- 1}
  else(data_frame_compl_pruned$Condition_1[i] <- 0)
}

#making sure the second set of conditions is met: GB022:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB022[i] == 1)
