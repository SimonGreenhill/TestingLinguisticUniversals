#Universal 331a. If the prefix position is the only possible one for a subject-affix, then  the object-affix also occupies this position in all forms (with the  exception of Wolio) or at least in some of them (no exceptions).

#s prefix > o prefix & ¬ o suffix (strong)

#(GB092:1 & GB091:0) > (GB094:1 & GB093:0)
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB092", "GB091", "GB094", "GB093"))
...
#making sure the second set of conditions is met: (GB094:1 & GB093:0)
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB094[i] == 1 | data_frame_compl_pruned$GB093[i] == 0)
