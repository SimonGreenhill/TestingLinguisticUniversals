#Universal 1027c. "There are correlations between following parameters:
...
#GB070:0 > GB083:0 | ((GB131:1 | GB132:1) & GB133:0)
...
#making sure the first set of conditions is met: GB070:0 
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB070[i] == 0)
  {data_frame_compl_pruned$Condition_1[i] <- 1}
  else(data_frame_compl_pruned$Condition_1[i] <- 0)
}

#making sure the second set of conditions is met: GB083:0 | ((GB131:1 | GB132:1) & GB133:0)

for(i in 1:nrow(data_frame_compl_pruned)){
  if(((data_frame_compl_pruned$GB131[i] == 1 | data_frame_compl_pruned$GB132[i] == 1) & data_frame_compl_pruned$GB133[i] == 0) | data_frame_compl_pruned$GB083[i] == 0)
