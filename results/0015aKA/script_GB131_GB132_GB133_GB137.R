#Universal 15a. Verbal modifiers like those for negation, causation, and reflexive or reciprocal are placed after verb roots in OV languages and before verb roots in VO languages.

#OV > V-Mod

#GB131:0 & (GB132:1 | GB133:1) >  GB137:1
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB131", "GB132", "GB133", "GB137"))
...
for(i in 1:nrow(data_frame_compl_pruned)){
  if((data_frame_compl_pruned$GB133[i] == 1 | data_frame_compl_pruned$GB132[i] == 1) & data_frame_compl_pruned$GB131[i] == 0)
  {data_frame_compl_pruned$Condition_1[i] <- 1}
  else(data_frame_compl_pruned$Condition_1[i] <- 0)
}

#making sure the second set of conditions is met:  GB137:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB137[i] == 1)
