#Universal 1612d. "In SOV languages, when the expected order of Modifier-Noun does not occur,  agreement [between noun and modifier] tends to appear.

#SOV & N-Dem > N-Dem agr

#GB131:0 & GB132:0 & GB133:1 & GB025:2 > GB171:1 | GB185:1
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB131", "GB132", "GB133", "GB025", "GB171", "GB185"))
...
#making sure the first set of conditions is met: GB131:0 & GB132:0 & GB133:1 & GB025:2 
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB131[i] == 0 & data_frame_compl_pruned$GB132[i] == 0 & data_frame_compl_pruned$GB133[i] == 1 & data_frame_compl_pruned$GB025[i] == 2)
  {data_frame_compl_pruned$Condition_1[i] <- 1}
  else(data_frame_compl_pruned$Condition_1[i] <- 0)
}

#making sure the second set of conditions is met:  GB171:1 | GB185:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB171[i] == 1 | data_frame_compl_pruned$GB185[i] == 1)
