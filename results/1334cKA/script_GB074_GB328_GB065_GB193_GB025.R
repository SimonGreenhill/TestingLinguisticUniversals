#Universal 1334c. "Prepositional Noun-Modifier Hierarchy (PrNMH): If a language is prepositional, then
...
#Adp-N & Rel-N & Gen-N & Adj-N > Dem-N

#GB074:1 &  GB328:1 & GB065:1 & GB193:1 > GB025:1
...
data_frame <- subset(x = GB_wide_strict, select = c("Language_ID", "GB074", "GB328", "GB065", "GB193", "GB025"))
...
#making sure the first set of conditions is met: GB074:1 &  GB328:1 & GB065:1 & GB193:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB074[i] == 1 & data_frame_compl_pruned$GB328[i] == 1 & data_frame_compl_pruned$GB065[i] == 1 & data_frame_compl_pruned$GB193[i] == 1)
  {data_frame_compl_pruned$Condition_1[i] <- 1}
  else(data_frame_compl_pruned$Condition_1[i] <- 0)
}

#making sure the second set of conditions is met: GB025:1
for(i in 1:nrow(data_frame_compl_pruned)){
  if(data_frame_compl_pruned$GB025[i] == 1)
