...
df_BT_n <- read_tsv(fns, show_col_types = FALSE, col_names = c("Glottocode", "Var1", "Var2"), id="Filename")
df_BT_n <- df_BT_n |> mutate(Universal = basename(dirname(Filename)))
...
universals_type <- read_tsv("universals_types.tsv", show_col_types = FALSE)

fns <- list.dirs(path = "2023_08_24_no_tree_fam_control_bfcluster", full.names = TRUE, recursive = TRUE)
