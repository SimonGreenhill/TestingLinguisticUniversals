...
showtext_auto()  # use system fonts
...
df.bayestraits <- readr::read_tsv("results/BT_results_summary/results.txt", show_col_types=FALSE) %>%
  ...
  dplyr::select(code, Universal.shorter,
                ...
                o95HDI_SIG) %>%
  dplyr::mutate(Universal.shorter = str_replace_all(Universal.shorter, ">", "⇒"))
...
ggsave(filename="figure2-bayesfactors-pointrange.pdf", width = 22, height = 27, units = "cm", dpi = 300, plot = p, device = cairo_pdf)
