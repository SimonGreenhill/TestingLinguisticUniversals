...
df_sp <- read_tsv("summary/df_brms_sp_posterior.tsv", show_col_types = F) %>%
  ...
  ungroup() %>%
  mutate(crosszero = median_l_95_CI <= 0 & median_u_95_CI >= 0) %>%
...
df_naive <- read_tsv("summary/df_brms_naive.tsv", show_col_types = FALSE) %>%
  filter(term == "fixed_V3") %>%
...
plot_bar <- function(df, title="xx", label_legend=FALSE, label_axis=FALSE, label_model=TRUE) {
    ...
    df$support <- factor(df$support, levels=c(
        "yes (supported, 95% CI excludes zero)",
        "no (not supported, 95% CI does not exclude zero)"))
    ggplot(df, aes(x = model, group=support, fill=support), alpha = 0.8) +
        geom_bar(show.legend = label_legend) +
        scale_fill_manual('Supported', values=c("steelblue", "lightgray")) +
        xlab(NULL) +
        scale_y_continuous(breaks=seq(0, 190, 25)) +
        coord_flip() +
        ggtitle(title) +
        theme_bw() +
...
ggsave(filename="figure1-barplot.png", height = 8, width = 18, plot=p.fig1)

grDevices::cairo_pdf(file="figure1-boxplot.pdf", height = 8, width = 18)
plot(p.fig1)
x <- dev.off()
